...
//gaoxuan --test
    ...
    if (mds_machine == machine_->machine_id()) {//gaoxuan --本地的话直接用Get取出,当前来看是能够取出来的
      ...
      LOG(ERROR)<<ParentDir(in.from_path())<<";"<<store_->Get(in.from_path(),10,&metadata_entry1)<<";"<<metadata_entry1.size()<<":"<<metadata_entry1;
    // If not local, get result from the right machine (within this replica).
      ...
      header->add_misc_string(ParentDir(in.from_path()).c_str(),strlen(ParentDir(in.from_path()).c_str()));//这一行也没问题
